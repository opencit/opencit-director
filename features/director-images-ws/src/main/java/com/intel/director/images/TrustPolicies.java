...
public class TrustPolicies {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(TrustPolicies.class);
    ...
    @GET
    public Response getTrustPoliciesData(@PathParam("trustPolicyId") String trustPolicyId) {
     ...
     * {"error": "Trust policy ID is not in a UUID format","error_code":{"errorCode":600,"errorDescription":"Validation failed"}}
     * 
     * In case of ID that does not exist
     * {"error": "Trust Policy does not exist for the given id","error_code": {"errorCode": 604,"errorDescription": "Record does not exist"}}
    	
    	In case the display name is not provided:
    	{"error": "Display name is empty or improper format. Name can contain only numbers, alphabets, no spaces and following characters ,;.@_-","error_code":{"errorCode":600,"errorDescription":"Validation failed"}}
     *                    </pre>
     * 
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @POST
    public Response updateTrustPolicy(@PathParam("trustPolicyId") String trustPolicyId,
	...
	if (StringUtils.isNotBlank(errors)) {
	    ...
	    monitorStatus.errorCode = ErrorCode.VALIDATION_FAILED;
	    return Response.status(Response.Status.BAD_REQUEST).entity(monitorStatus).build();
	}
	...
	if (trustPolicyByTrustId == null) {
	    ...
	    monitorStatus.errorCode = ErrorCode.RECORD_NOT_FOUND;
	    return Response.status(Response.Status.BAD_REQUEST).entity(monitorStatus).build();
	}
	...
	try {
	    ...
	    monitorStatus.errorCode = ErrorCode.REQUEST_PROCESSING_FAILED;
     ...
     * {"error": "No image with id : 7A871544-87E0-42A1-B545-CDC6477DC2F9 exists.","error_code": {"errorCode": 604,"errorDescription": "Record does not exist"}}
     *  
     * 2) When image id provided is not in correct format: 
     * {"error": "Image id is empty or not in uuid format","error_code":{"errorCode":600,"errorDescription":"Validation failed"}}
	...
	if (imageInfo == null) {
	    ...
	    genericResponse.error = "Templates can only be applied to non virtualized servers.";
	...
	try {
	    importPolicyTemplateResponse = imageService.importPolicyTemplate(req.getImage_id());
	} catch (DirectorException e) {
     ...
     * 
     * In case of error:
     * {"deleted":false; "error":"Error in deleting trust policy: <UUID>","error_code": {"errorCode": 601,"errorDescription": "Request processing failed"}}
     *                    </pre>
     * 
     * @param trustPolicyId
     * @return GenericResponse
    ...
    @DELETE
    @Produces(MediaType.APPLICATION_JSON)
    public Response deletePolicy(@PathParam("trustPolicyId") String trustPolicyId) {
	...
	try {
	    ...
	    response.setError("Error in deleting trust policy : " + trustPolicyId);
	}
    ...
    public Response downloadPolicyForTrustPolicyId(@PathParam("trustPolicyId") String trustPolicyId) {
	GenericResponse genericResponse = new GenericResponse();
	if (!ValidationUtil.isValidWithRegex(trustPolicyId, RegexPatterns.UUID)) {
	    genericResponse.error = "Trust Policy id is empty or not in uuid format";
	    genericResponse.errorCode = ErrorCode.INAVLID_ID;
	...
	response.header("Content-Disposition",
		"attachment; filename=policy_" + trustPolicy.getImgAttributes().getImage_name() + ".xml");
