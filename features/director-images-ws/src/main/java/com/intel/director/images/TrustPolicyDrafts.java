...
public class TrustPolicyDrafts {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(TrustPolicyDrafts.class);
	...
	listTrustPolicyDrafts.setTrust_policy_drafts(imageService.getTrustPolicyDrafts(null));
    ...
    @GET
    @Produces(MediaType.APPLICATION_JSON)
    public Response getPolicyDraft(@PathParam("trustPolicyDraftId") String trustPolicyDraftId) {
	...
	try {
	    createTrustPolicyMetaDataResponse = imageService.getPolicyMetadata(trustPolicyDraftId);
	    if (createTrustPolicyMetaDataResponse == null) {
		genericResponse.errorCode = ErrorCode.RECORD_NOT_FOUND;
		genericResponse.error = "Trust policy details not found for draft id: "+trustPolicyDraftId;
		return Response.status(Response.Status.NOT_FOUND).entity(genericResponse).build();
	    }
	} catch (DirectorException e) {
	    createTrustPolicyMetaDataResponse = new CreateTrustPolicyMetaDataResponse();
	    genericResponse.setStatus(Constants.ERROR);
     ...
     * 
     * A sample patch looks like this: <patch> <add sel=
     * "<node selector>" ><File path="<file path>"></add> <remove sel=
     * "<node selector>" ></remove> </patch>
     * 
     ...
     * }
     * 
     * In case of validation error like incorrect uuid:
     * Output: {"status":"Error", details:"Trust Policy Draft Id is empty or not in uuid format","error_code":{"errorCode":600,"errorDescription":"Validation failed"}}
     * 
     *                    </pre>
     * 
     * @param imageId
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public Response editPolicyDraft(@PathParam("trustPolicyDraftId") String trustPolicyDraftId,
	...
	try {
	    policyDraft = imageService.editTrustPolicyDraft(trustPolicyDraftEditRequest);
	...
	return Response.ok(policyDraft).build();
    }
     ...
     * {"error":"Unable to sign the policy with attestation service","error_code": {"errorCode": 601,"errorDescription": "Request processing failed"}}
     *                    </pre>
     * 
     * 
     * @param CreateTrustPolicyMetaDataRequest
     *            CreateTrustPolicyMetaDataRequest containing
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @POST
    public Response createTrustPolicy(CreateTrustPolicyMetaDataRequest createPolicyRequest) {
	...
	try {
	    ...
	    createTrustPolicyDraftResponse.setErrorCode(ErrorCode.REQUEST_PROCESSING_FAILED);
	    log.error("Error creating policy from draft for image : " + createPolicyRequest.image_id, de);
	}
     ...
     *                    with output:-
     * 
     *                    {"error": "Invalid image id provided","error_code": {"errorCode": 604,"errorDescription": "Record does not exist"}}
     * 
     * @param createTrustPolicyMetaDataRequest
     ...
     * "image_upload_status":"Complete","image_Location":"/mnt/images/vm/"}}
     * 
     *                    </pre>
     * 
     * @param imageId
	...
	if (trustPolicyByTrustId == null) {
	    genericResponse.error = "No trust policy exists for image with id : " + req.getImage_id() + " exists.";
	    genericResponse.errorCode = ErrorCode.RECORD_NOT_FOUND;
	    return Response.status(Response.Status.BAD_REQUEST).entity(genericResponse).build();
	}
     ...
     * Output: { "error": "Trust Policy Draft Id does not exist", "deleted": false,"error_code": {"errorCode": 604,"errorDescription": "Record does not exist"}}
     *                    </pre>
     * 
     * @param trustPolicyDraftId
    ...
    @DELETE
    @Produces(MediaType.APPLICATION_JSON)
    public Response deletePolicyDraft(@PathParam("trustPolicyDraftId") String trustPolicyDraftId) {
    ...
    /**
     * 
     * This method looks into the MW_TRUST_POLICY_DRAFTS table and gets the
     * policy draft string and sends it as an xml content to the user.
     * 
     * In case the policy draft is not found for the trust policy draft id, HTTP
     * 404 is returned
     * 
    ...
    public Response downloadPolicyForTrustPolicyDraftId(@PathParam("trustPolicyDraftId") String trustPolicyDraftId) {
	GenericResponse genericResponse = new GenericResponse();
	if (!ValidationUtil.isValidWithRegex(trustPolicyDraftId, RegexPatterns.UUID)) {
	    genericResponse.error = "Trust Policy Draft id is empty or not in uuid format";
	    return Response.status(Response.Status.BAD_REQUEST).entity(genericResponse).build();
	...
	if (trustPolicyDraft == null) {
	    genericResponse.error = "No trust policy draft found for id: "+trustPolicyDraftId;
	    genericResponse.errorCode = ErrorCode.RECORD_NOT_FOUND;
	    return Response.status(Response.Status.NOT_FOUND).entity(genericResponse).build();
	}
	ResponseBuilder response = Response.ok(trustPolicyDraft.getTrust_policy_draft());
	response.header("Content-Disposition",
		"attachment; filename=policy_" + trustPolicyDraft.getImgAttributes().getImage_name() + ".xml");
	...
	try {
	    ...
	    versionedDisplayNameForDockerImage = trustPolicyService.getVersionedDisplayNameForDockerImage(imageInfo);
	} catch (DirectorException e) {
