...
public class ImageActions {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(ImageActions.class);
     ...
     * being processed.
     * 
     * Status of the action tasks can be Incomplete,Complete or Error depending
     * on result of execution
     * 
	...
	try {
		...
		return Response.ok(actionService.fetchImageAction(actionId)).build();
	    } else {
		genericResponse.error = "No image action found for id: " + actionId;
     ...
     * 
     * In case of error:
     * {"error": "Image does not exist for id: 1AFBA2F5-C02E-420E-9842-C455BB35B332", "error_code": {"errorCode": 604,"errorDescription": "Record does not exist"}}
     * 
     *                    </pre>
     * 
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @POST
    public Response createImageAction(ImageActionRequest imageActionRequest) {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @DELETE
    public Response deleteImageAction(@PathParam("actionId") String actionId) {
	...
	try {
	    ...
	    imageDeleteResponse.errorCode = ErrorCode.REQUEST_PROCESSING_FAILED;
     ...
     *                    id: { "error": "Image with the id do not exist", "error_code": {"errorCode": 604,"errorDescription": "Record does not exist"}}
     * 
     * @param imageId
	...
	try {
	    ...
	    genericResponse.error = "GetImageDetails failed with error: " + e1.getMessage();
	    genericResponse.errorCode = ErrorCode.REQUEST_PROCESSING_FAILED;
	...
	if (imageInfoResponse == null) {
	    ...
	    genericResponse.errorCode = ErrorCode.RECORD_NOT_FOUND;
	    log.error("Image with the id: {} do not exist", imageId);

	    return Response.status(Response.Status.NOT_FOUND).entity(genericResponse).build();
	}
	try {
	    imageActionResponseList.setImageActionResponseList(actionService.getImageActionHistory(imageId));
	} catch (Exception e) {
