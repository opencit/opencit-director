...
function goToUploadToImageStorePage(){
	 ...
	 $("#upload_to_image_store_redirect").load("upload_imagestore_direct.html");
...
function backButton(){
	...
	var previousStepNum=eval(stepNum)-1;
	 ...
	 $("#"+active_wizard_page+"_step_"+previousStepNum).removeClass("done");
	 $("#"+active_wizard_page+"_step_"+previousStepNum).addClass("selected");
...
function nextButton(){
	...
	var nextStepNum=eval(stepNum)+1;
	 ...
	 var html_url= $("#"+active_wizard_page+"_step_"+nextStepNum).attr('href');
	 var htmlpage = html_url.substring(1);
		
	 var isEmpty=!$.trim( $("#"+active_wizard_page+"_content_step_"+nextStepNum).html());
		
	 if(isEmpty==true){
		 $("#"+active_wizard_page+"_content_step_"+nextStepNum).load(htmlpage);
	 }

	 $("#"+active_wizard_page+"_step_"+nextStepNum).addClass("selected");
	 $("#"+active_wizard_page+"_step_"+stepNum).removeClass("selected");
	 $("#"+active_wizard_page+"_step_"+stepNum).addClass("done");
	 $("#"+active_wizard_page+"_step_"+nextStepNum).removeClass("disabled");
...
function downloadPolicy(imageid,trust_policy_id){
	window.location="/v1/images/"+trust_policy_id+"/download";
