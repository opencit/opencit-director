...
function ResourceLoader() {
        ...
        if (self.html[url]) {
            ...
            $.ajax({
                ...
                beforeSend: function (xhr, settings) {
                    xhr.meta = {"url": url};
                }, // define a new field "meta" in the xhr with our original request url
    ...
    self.postLoadJS = function (request) {
        ...
        if (request.statusTimerId) {
            clearTimeout(request.statusTimerId);
            request.statusTimerId = null;
        }
...
//            if( self.js[url.status = "error" ) {
    ...
    self.findPrefix = function (strs) {
        ...
        strs = strs.filter(function (item) {
