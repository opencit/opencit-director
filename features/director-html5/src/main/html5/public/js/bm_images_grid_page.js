        ...
        success: function(result) {
            ...
            for (i = 0; i < images.length; i++) {
                ...
                if (images[i].trust_policy_draft_id != null) {
                    self.gridData.trust_policy = self.gridData.trust_policy + "<a href=\"#\" title=\"Edit Policy\"><span class=\"glyphicon glyphicon-edit\" id=\"bmlive_edit_row_" + i + "\" title=\"Edit Policy\"  onclick=\"editPolicyForBMLive('" + images[i].id + "','" + images[i].image_name + "','" + images[i].trust_policy_draft_id + "')\"></span></a>";

                } else if (images[i].trust_policy_id != null) {
                    self.gridData.trust_policy = self.gridData.trust_policy + "<a href=\"#\" title=\"Edit Policy\" ><span class=\"glyphicon glyphicon-edit\" id=\"bmlive_edit_row_" + i + "\"  title=\"Edit Policy\" onclick=\"editPolicyForBMLive('" + images[i].id + "','" + images[i].image_name + "','')\"></span></a>";
                }
                ...
                if (images[i].trust_policy_id != null || images[i].trust_policy_draft_id != null) {
                    self.gridData.trust_policy = self.gridData.trust_policy + "&nbsp;<a href=\"#\"><span class=\"glyphicon glyphicon-trash\" id=\"bmlive_delete_row_" + i + "\"  title=\"Delete Policy\" onclick=\"deletePolicyBM('" + images[i].id + "')\"></span></a>";
                }

                self.gridData.trust_policy = self.gridData.trust_policy + "</div>";

                self.gridData.image_upload = "";
                self.gridData.image_upload += "&nbsp;" + "<a href=\"#\" title=\"Upload\" ><span class=\"glyphicon glyphicon-open\" id=\"bmlive_push_row_" + i + "\" title=\"Push To  Host\" onclick=\"pushPolicyToHost('" + images[i].id + "','" + images[i].image_name + "','" + images[i].trust_policy_id + "')\" ></span></a>";
