...
function SelectDirectoriesViewModel() {
    ...
    self.selectDirectoriesSubmit = function(loginFormElement) {
        $("#createDockerPolicyDirNext").prop('disabled', true);
...
function toggleState(node) {
    ...
    if ($(node).hasClass('fa-unlock')) {
		$("#resetRegexDockerBtn").prop("disabled", true);
	} else {

		$("#resetRegexDockerBtn").prop("disabled", false);
	}
...
function backToDockerFirstPage() {
    clearInterval(refreshIntervalId);
    var mountimage = {
        "id": current_image_id
    }
