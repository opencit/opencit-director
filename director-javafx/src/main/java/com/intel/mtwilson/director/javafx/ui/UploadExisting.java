...
//        
        ...
        grid.add(imageFormat, 0, 4);
        grid.add(imageFormatChoiceBox, 1, 4);
    ...
    private String UploadNow(String trustPolicyLocation, String imageLocation) throws ImageStoreException, Exception {
        JAXB jaxb = new JAXB();
        TrustPolicy trustPolicyObj = jaxb.read(new String(Files.readAllBytes(Paths.get(trustPolicyLocation))), TrustPolicy.class);
        //create config map
        Map<String, String> confInfo = new HashMap<>();
        confInfo.put(Constants.IMAGE_NAME, imageNameTField.getText());
        confInfo.put(Constants.IMAGE_ID, trustPolicyObj.getImage().getImageId());
        confInfo.put(Constants.IMAGE_TYPE, imageFormatChoiceBox.getValue().toString());
        confInfo.put(Constants.IMAGE_LOCATION, imageLocation);
        for(Map.Entry<String, String> entry:confInfo.entrySet()){
            log.debug("key value are : {} {}",entry.getKey(), entry.getValue());
        }
        String message = new UserConfirmation().setImagePropertiesAndUploadToGlance(confInfo, trustPolicyLocation, uploadExistingStage);
        return message;
    }
